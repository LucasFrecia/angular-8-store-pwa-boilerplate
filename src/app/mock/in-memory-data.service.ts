...
import { TransportItemModel } from '../views/transports/transport-selector/store/transport-selector.model';

@Injectable({
    providedIn: 'root'
})
export class AppInMemDataService {

    createDb() {

        /** Feature: Transport Selector */
        const transportSelector = this.getTransportItems();

        return {
            'transports-list': transportSelector,
        };
    }

    private getTransportItems(): TransportItemModel[] {
        const items = [
            {
                id: 1,
                type: 'car',
